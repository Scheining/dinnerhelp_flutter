-- Migration: Add archive and delete functionality to messaging tables
-- Date: 2025-01-05
-- Purpose: Support swipe actions for archiving and deleting conversations
...
-- Users can only see non-deleted conversations they're part of
CREATE POLICY "Users can view their non-deleted inquiries" ON inquiries
  FOR SELECT
  USING (
    auth.uid() IN (user_id, chef_id) 
    AND is_deleted = false
  );

CREATE POLICY "Users can update archive status of their inquiries" ON inquiries
  FOR UPDATE
  USING (auth.uid() IN (user_id, chef_id))
  WITH CHECK (auth.uid() IN (user_id, chef_id));